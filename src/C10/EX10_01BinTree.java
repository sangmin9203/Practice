...
public class EX10_01BinTree<K, V> {
	...
	//생성자
	public EX10_01BinTree() {
		root = null;
	}
	
	//생성자
	public EX10_01BinTree(Comparator<? super K> c) {
		this();
		comparator = c;
	}
	...
	public V search(K key) {
		...
		while(true) {
			if(p == null) {
				return null;						// 더 이상 진행하지 않으면 검색실패
			}										
	...
	public boolean remove(K key) {
		...
		if(p.left == null) {								// p에는 왼쪽 자식이 없음
			...
			if(p == root) {
				...
				parent.right = p.left;						// 부모의 오른쪽 포인터가 왼쫀 자식을 가리킴
			}
		} else {
			parent = p;
